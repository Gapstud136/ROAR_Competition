...
# --- fixes -----------------------------------------------------------------
# A: reset stale internal state after the runner teleports us back to spawn.
#    Without this, one collision poisons every remaining lap.
...
class RoarCompetitionSolution:
    ...
    def section_from_index(self, idx: int) -> int:
        """
        Map a waypoint index to its section by range rather than by proximity.
        Section k runs from section_indeces[k] up to the next section start.
        Section 0 wraps past the end of the lap (2611 -> 2626 -> 0 -> 322).
        """
        starts = self.section_indeces
    ...
    def recover_from_respawn(self, vehicle_location):
        """
        The runner teleports us to the spawn point and zeroes its own lap
        progress after a hard collision, but leaves this object untouched.
        Re-seed everything that is position-dependent.
        """
        self.current_waypoint_idx = findClosestIndex(
            vehicle_location, self.maneuverable_waypoints
        )
        self.current_section = self.section_from_index(self.current_waypoint_idx)
        # WaypointLine only searches +/-50 around prev_index, so a stale value
        # after a teleport makes every snap-to-line target meaningless.
        # Scan the whole line here -- find_closest_index() is windowed and can
        # fall through to returning a *location* instead of an index.
        line = self.waypoint_line.line_locations
        best_i, best_d = 0, float("inf")
        for i, loc in enumerate(line):
            d = np.linalg.norm(vehicle_location[:2] - loc[:2])
            if d < best_d:
                best_d, best_i = d, i
        self.waypoint_line.prev_index = best_i
        self.previous_brake = False
        self.s3_mult = 1
        self.previous_location = vehicle_location
        print(
            f"[RESPAWN] recovered -> wp {self.current_waypoint_idx} "
            f"section {self.current_section} "
            f"lineidx {self.waypoint_line.prev_index}"
        )
    ...
    async def step(self) -> None:
        ...
        # --- teleport detection (must run before anything position-dependent)
        if FIX_RESPAWN_RESET and self.previous_location is not None:
            jump = float(
                np.linalg.norm(vehicle_location[:2] - self.previous_location[:2])
            )
            if jump > RESPAWN_JUMP_M:
                print(f"[RESPAWN] jump {jump:.1f} m detected at tick {self.num_ticks}")
                self.recover_from_respawn(vehicle_location)
            elif jump > 5.0:
                # normal motion at 300 km/h is ~4.2 m/tick, so anything above
                # this is worth seeing while we debug
                print(f"[BIGSTEP] {jump:.1f} m at tick {self.num_ticks}")
        ...
        if FIX_SECTION_RANGE:
            new_section = self.section_from_index(self.current_waypoint_idx)
            if new_section != self.current_section:
                if new_section == 0 and self.lapNum != 3:
                    self.lapNum += 1
...
#                 )
        ...
        # Must happen every tick regardless of useDebug -- the teleport check
